...
const solvePuzzle = (input) => {
    ...
    const screen = getScreenArr(timeline);
    const crt = [];
    for (let line of screen) {
        const pixels = line.map((pixel) => pixel[1]);
        const translated = pixels.map((pixel, i) => Math.abs(pixel - i) < 2 ? 'X' : '.');
        crt.push(translated);
    }
